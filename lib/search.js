...
(function(){
  var Charm, vw, ttys, migemo, e, bytesToString, DOWN, UP, BACKSPACE, CTRLC, CTRLD, CTRLK, CTRLJ, CTRLN, CTRLP, ENTER, DUMMY_HIT, search, searchUsingDefault, searchCore, cleanupScreen, drawScreen, drawRow, getHits, queryHits, SearchyMatch, readStdinAsLinesThen, noEmpty, searchFromStdin, out$ = typeof exports != 'undefined' && exports || this;
  Charm = require('charm');
  ...
  drawRow = function(charm, rows, columns, needle, selRow, matches, row){
    ...
    if (!matches[row].hit) {
      return charm.write(txt + repeatString$(' ', padLength));
    } else {
      hit = matches[row].hit;
