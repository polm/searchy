...
(function(){
  var Charm, vw, ttys, debounce, fs, migemo, e, bytesToString, DOWN, UP, BACKSPACE, CTRLC, CTRLD, CTRLK, CTRLJ, CTRLL, CTRLN, CTRLP, ENTER, search, searchUsingDefault, searchCore, updateResults, cleanupScreen, drawNeedle, drawScreen, drawRow, getHits, queryHits, SearchyMatch, readStdinAsLinesThen, noEmpty, searchFromStdin, out$ = typeof exports != 'undefined' && exports || this;
  Charm = require('charm');
  ...
  searchCore = function(items, cb, defaultCb, matcher){
    ...
    ttys.stdin.on('data', function(chunk){
      var rows, cols, i$, to$, ii, that, oldHeight, countString;
      if (state.height < 0) {
        state.height = 0;
      }
      ...
      charm.cursor(false);
      if (chunk[0] === +BACKSPACE && chunk.length > 1) {
        for (i$ = 0, to$ = chunk.length; i$ < to$; ++i$) {
          ii = i$;
      ...
      case ENTER:
        ...
        break;
      case BACKSPACE:
        fs.appendFileSync('log', "backspace/" + bytesToString(chunk) + ", " + state.needle + "\n");
        state.height = 0;
        if (state.needle.length > 0) {
          state.needle = state.needle.substr(0, state.needle.length - 1);
        }
        drawNeedle(charm, state.needle);
        updateResults(state, items, charm, rows, cols, matcher);
        break;
      case CTRLL:
        drawNeedle(charm, state.needle);
        updateResults(state, items, charm, rows, cols, matcher);
      ...
      default:
        if (0 !== that.indexOf("27.91.")) {
          ...
          state.height = 0;
          fs.appendFileSync('log', "key:" + chunk + "/" + bytesToString(chunk) + ", " + state.needle + "\n");
        }
        drawNeedle(charm, state.needle);
        updateResults(state, items, charm, rows, cols, matcher);
      }
    ...
    refresh = function(){
      return ttys.stdin.emit('data', [CTRLL]);
    };
    refresh();
    return {
      items: items,
  ...
  updateResults = function(state, items, charm, rows, cols, matcher){
    getHits(state, items, rows, matcher);
    fs.appendFileSync('log', "got hits, " + state.needle + "\n");
    return drawScreen(charm, rows, cols, state.needle, state.height, state.matches);
  };
  updateResults = debounce(updateResults, 50, {
    leading: true
  });
  ...
  drawNeedle = function(charm, needle){
    charm.display('reset');
    charm.position(1, 1);
    charm.erase('line');
    return charm.write("query: " + needle);
  };
  drawScreen = function(charm, rows, columns, needle, selRow, matches){
    var i$, to$, row, results$ = [];
    drawNeedle(charm, needle);
  ...
  drawRow = function(charm, rows, columns, needle, selRow, matches, row){
    ...
    if (!matches[row].hit) {
      return charm.write(vw.truncate(txt + repeatString$(' ', padLength), columns - 1, ''));
    } else {
      hit = matches[row].hit;
